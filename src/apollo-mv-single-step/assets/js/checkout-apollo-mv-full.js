 ...
 * the Apollo MV checkout — a <button class="os-card__variant-dropdown-toggle"> plus
 ...
 * default map below (same key style as the MV TierController: slug from label).
 *
...
// Checkout reveal — show only the MV selector + "Add to Cart" first; reveal the
...
(function initCheckoutReveal() {
  ...
  document.querySelectorAll('.checkout-form--reveal:not(.is-revealed) [data-checkout-reveal-panel]').forEach(function (reveal) {
    reveal.setAttribute('inert', '');
  });
  document.querySelectorAll('[data-checkout-reveal-trigger]').forEach(function (btn) {
    btn.addEventListener('click', function () {
      var form = btn.closest('.checkout-form--reveal');
      if (!form) return;
      form.classList.add('is-revealed');
      var reveal = form.querySelector('[data-checkout-reveal-panel]');
      if (reveal) {
...
(function initReviewsSliders() {
  ...
  document.querySelectorAll('[data-component="reviews-swiper"]').forEach(function (el) {
    new Swiper(el, {
      slidesPerView: 'auto',
      spaceBetween: 16,
      grabCursor: true,
