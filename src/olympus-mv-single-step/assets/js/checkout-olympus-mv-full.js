...
function syncMvSlotDiscountLines() {
  const card = document.querySelector(
    '[data-next-bundle-card][data-next-selected="true"]',
  );
  if (!card) return;
  ...
  const renderedChildren = (src) =>
    src
      ? [...src.children].filter((el) => el.tagName !== 'TEMPLATE')
      : [];

  const offerHtml = renderedChildren(offerSrc)
    .map((el) => el.outerHTML)
    .join('');
  const voucherHtml = renderedChildren(voucherSrc)
    .map((el) => el.outerHTML)
    .join('');

  document.querySelectorAll('.mv-slot-offer-line').forEach((el) => {
  ...
  document.querySelectorAll('.mv-slot-voucher-line').forEach((el) => {
    ...
    const show =
      voucherSrc?.classList.contains('next-discounts-has-items') ?? false;
