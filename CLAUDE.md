...
**`upsell-single`** (`apollo` today; propagation to the other families pending) — single-offer upsell: one package, fixed price, accept or skip, no tiers. Page `upsell-single.html` + partial `upsell-single-offer.html` (catalog component `upsell-single-offer`, surface token `single`). Frontmatter: `upsell_offer` (+ optional `voucher` → hidden linked bundle selector, prices read `bundle.<selector_id>.*`), `offer_features`, `offer_comparison`, `offer_steps`, `guarantee_note`, `countdown_timer`, `announcement_text`. Demo copy is a protection plan; every string is an override.

**`countdown-timer.html`** (apollo + apollo-mv-single-step) — slim urgency bar, **not** the `<promo-timer>` web component the other families use. Two modes from `countdown_timer` data: `duration` (seconds) → SDK `data-next-timer`, persisted per `persistence_id` (upsell pages); `start` (`"MM:SS"`) → legacy `[data-next-element="timer"]` driven by `checkout.js` (checkout). Renders nothing when absent. CSS classes are `.countdown-timer*` in core.
