...
**`upsell-single`** (all eight commerce families) — single-offer upsell: one package, fixed price, accept or skip, no tiers. On **apollo** the page is composed from the shared upsell partial set (`upsell/offer.html` with `upsell_offer.type: single` + `upsell/body.html` + `upsell/closing-cta.html`). The other seven families still ship page `upsell-single.html` + monolithic partial `upsell-single-offer.html` (catalog component `upsell-single-offer`, surface token `single`). Frontmatter: `upsell_offer` (+ optional `voucher` → hidden linked bundle selector, prices read `bundle.<selector_id>.*`), `offer_features`, `offer_comparison`, `offer_steps`, `guarantee_note`, `countdown_timer`, `announcement_text`, `pricing_mode` (`discounted` default: struck compare + % badge + price · `compare_at` · `full_price`; the **demeter** copy defaults `full_price` per its brand contract and its demo page opts into `discounted`). Demo copy is a protection plan; every string is an override. Drone-campaign families demo package 12 + `LIFE50`; MV families demo package 77 with no voucher.

### Composable upsell partials (`apollo` — propagate to the other families next)

Every apollo upsell page is assembled from **one interchangeable partial set**, selected and filled from frontmatter. A bundle upsell, the single-offer upsell and the long-form VSL upsell are the same includes with different data (`upsell-bundle-stepper.html` vs `upsell-vsl.html` share `upsell_offer.type: stepper`). Page order:
...
- **MV**: `offer_type='mv'` branches (configurable hidden cards, slot `<template>` with `slot_personalization`, qty pills, `order_personalization`, `#bundle-slots-stage`) are written into `upsell/offer-selector.html` / `-controls.html` so the same files can land in the MV families unchanged; untested until propagated (needs `upsells-mv.js` + `upsell_mv_tiers`).
