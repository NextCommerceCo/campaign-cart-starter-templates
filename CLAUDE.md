...
- **Analytics partials — shared across all three layouts (DRY):** the GTM + Meta Pixel snippets live in two per-template partials, `_includes/analytics-head.html` (head loaders) and `_includes/analytics-body.html` (`<noscript>` fallbacks), pulled into `base.html`, `base-presell.html`, and `base-landing.html` via `{% campaign_include %}` — same pattern as `meta-social.html`, so the three layouts never drift. Previously each block was inlined in all 3 layouts × 8 template families (24 copies each, byte-identical) — a tracking change now edits the **partial** (8 files), not 24 layouts. Both partials keep the `{% unless environment == "development" %}` + per-id `!= ""` gating; `src/landing/` is excluded (commented-out examples only). When adding a new channel, edit the partials, not the layouts.
