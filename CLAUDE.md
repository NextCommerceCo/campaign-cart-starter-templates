...
- **`upsell_layout`** — `style: card` (default: the bundle look — band overlap, shadowed container, content card with bullets/ships-info/decline) or `style: box` (the upsell-single look — bordered `upsell-container--box` in a surface section, image left `cc-medium`, centered `cc-fluid` copy, rating-first heading with the % badge, inline `Yours Today` price, `cc-sm` button with the live price, no bullets/ships-info/decline unless `show_bullets` / `show_decline`); `media_width: narrow | medium | wide` (box style column width), `media_align: center | top | sticky` (sticky needs `boxed: false`; the shadowed container clips it), `boxed` (false = no card/shadow; pair with `upsell_hero.variant: card`), `media`, `media_position` (`right` default = content left; `left` adds `cc-media-left`), `show_bullets`, `show_ships_info`, `show_payment_logos`.
