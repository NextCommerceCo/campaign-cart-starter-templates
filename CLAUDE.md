...
SDK 0.4.26 ("Unique Line Items by Properties") lets a customer attach custom text to a product (monogram, jersey name, gift message). Two slots with the **same `package_id` but different properties stay as separate cart lines and separate order lines** instead of merging into one with a higher quantity. Properties are forwarded through the full order lifecycle — `/calculate`, create-cart (ProspectCartEnhancer), create-order, express checkout, test orders — with no extra config.
- **Per-slot personalization** → `data-next-property-key` on an input inside a bundle slot. This is the half that produces **unique line items**, and it is the **only** declarative path to per-line *distinct* values — `data-next-property-key` is read **only inside slot elements** (the enhancer queries `slotEl`), so it does nothing outside the slot system. **Reference example:** a "Custom text" input inside the slot `<template>` in `olympus-mv-single-step/_includes/mv-configurable-selector.html` and `olympus-mv-two-step/_includes/mv-slot-stage.html` — the SDK clones the template per unit, so two units of the same package with different text become two separate lines.
  - **Toggleable via the `slot_personalization` frontmatter object** (MV-only; passed as an include arg). Fields: `enabled`, `property_key` (→ order line-item attribute name), `label`, `optional` (appends "(optional)"), `placeholder`, `maxlength`; partial defaults apply when omitted. Defaults shipped: **`olympus-mv-single-step/checkout.html` `enabled: true`** (live showcase); **`olympus-mv-two-step/select.html` `enabled: false`** (clean reference — the two-step slot template renders on the `select` step, so the input/frontmatter live there, not on its `checkout.html`).
  - **Requires a genuinely variant-configurable (MV) product — do NOT flip `configurable:true` on a plain product just to harvest personalization slots.** The slot system explodes an item into per-unit slots only when `configurable:true` **and** `quantity > 1`, and each configurable slot then **gates checkout on variant resolution**: the enhancer blocks (`needsVariant = slots.some(s => s.configurable && !s.variantSelected)` → `_getSelectedBundleItems` returns null) until `variantSelected` flips true, which happens only when the active package carries `product_variant_attribute_values` or the shopper picks a matching variant combo. A no-variant product never flips it, so a "personalization-only slots" setup bricks checkout. Decoupling slot personalization from variant selection would be an SDK enhancement request.
  - A tier-*swap* selector (e.g. `olympus`) has no slot machinery at all — `data-next-property-key` is never scanned there; use `data-next-default-property-key` (below).
- **Order-wide value** → `data-next-default-property-key` on an input outside the bundle (one gift message for the whole order). Works on any checkout, including tier-swap selectors — **reference example:** the "Add a Gift Message" field on `olympus/checkout.html`.
- **Render to customer** → add `[data-next-item-properties]` + `<template>` inside the cart-summary line template (`{property.key}`/`{property.value}`). Already wired in both `olympus/_includes/cart-summary01.html` and `olympus-mv-single-step/_includes/cart-summary01.html`.
- Opt-in and purely additive — templates without these attributes are unaffected. Existing `data-next-package-sync` / `data-next-product-sync` behavior is unchanged.
- **Not supported on post-purchase upsells (as of 0.4.26).** The upsell accept path (`POST /orders/{ref}/upsells/` via `addUpsell`) sends only `package_id` + `quantity` — verified across every upsell-accept builder in the dist (bundle, toggle, single, and the public `next.addUpsell()` API). A `data-next-property-key` on an upsell offer (`upsell-*-offer.html`) is silently ignored, so no upsell example exists. Properties attach only at checkout / create-order (and `/calculate`, create-cart, express checkout, test orders). Personalize at checkout instead.
