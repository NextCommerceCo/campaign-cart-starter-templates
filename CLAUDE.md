...
**`upsell-single`** (all eight commerce families) — single-offer upsell: one package, fixed price, accept or skip, no tiers. Composed from the shared upsell partial set (`upsell/offer.html` with `upsell_offer.type: single` + `upsell/body.html` + `upsell/closing-cta.html`; surface token `single`). Frontmatter: `upsell_offer` (+ optional `voucher` → hidden linked bundle selector, prices read `bundle.<selector_id>.*`), `offer_features`, `offer_comparison`, `offer_steps`, `guarantee_note`, `countdown_timer`, `announcement_text`, `pricing_mode` (`discounted` default: struck compare + % badge + price · `compare_at` · `full_price`; the **demeter** copy defaults `full_price` per its brand contract and its demo page opts into `discounted`). Demo copy is a protection plan; every string is an override. Drone-campaign families demo package 12 + `LIFE50`; MV families demo package 77 with no voucher.

### Composable upsell partials (all eight families)

Every upsell page in every family is assembled from **one interchangeable partial set** (`src/<family>/_includes/upsell/`, byte-identical across families except demeter's `full_price` pricing default — see `intentionalVariants`), selected and filled from frontmatter. A bundle upsell, the single-offer upsell and the long-form VSL upsell are the same includes with different data (`upsell-bundle-stepper.html` vs `upsell-vsl.html` share `upsell_offer.type: stepper`). Page order:
...
- **`pricing_mode`** lives in `upsell/offer-prices.html`: `discounted` (default; Retail struck | Offer + % OFF) · `compare_at` · `full_price` · `unit_total` · `unit_only`. Same contract as Demeter's brand contract (its copy defaults `full_price` when propagated). `bundle.<sel>.unitPrice` is not a reliable remote path — prefer `discounted` on bundle offers. `retail_shipping` / `offer_shipping` `""` hide those lines. When `upsell_offer.heading_prices: true` the rows are skipped (the heading strip already shows the price) unless `price_rows: true`.
...
- Surfaces + partial defaults (per contract): upsell offers (demeter's copies of `upsell/offer-prices.html` and `upsell/offer-heading.html`, reached by every offer type through the shared `upsell/offer.html` shell) default **full_price** — exactly one visible price row adjacent to the accept control; `bump-check01.html` defaults **full_price** (one `unitPrice`/ea row); `editorial-tier-selector.html` defaults **discounted** (current compare-unit-total render), with per-card override via `bundles[].pricing_mode`.
