...
upsell/offer.html          upsell_offer.type: tier-cards | tier-pills | stepper | single | mv   ← the shell (include arg offer_type= overrides)
  ...
  └ [data-next-upsell="offer"]  (the ONLY SDK offer wrapper on the page)
      ...
      ├ content column: upsell-offer-heading (pretitle · title · rating · optional heading_prices strip) → upsell-video (own include, when upsell_video is set) → upsell-offer-bullets → upsell-offer-controls (the ONLY per-type piece) → upsell-offer-prices (pricing_mode) → upsell-cta → upsell-guarantees
...
upsell/secondary-offer.html  upsell_secondary.variant: cta | card — accept/skip are data-upsell-proxy buttons
footer.html
```

Rules that keep the SDK happy:
- **One offer wrapper per page.** `UpsellEnhancer` resolves the bundle to add from the *first* `[data-next-bundle-selector]` inside its own `[data-next-upsell="offer"]`, so the closing CTA never gets a second wrapper. `upsell/secondary-offer.html` renders `data-upsell-proxy="add|skip"` and `initUpsellProxyActions()` (`upsells.js`, self-initialising) clicks the real in-offer action. Display-only / voucher selectors stay outside the wrapper; the hidden state selector is rendered first inside it.
...
- **Shims**: `upsell-bundle-stepper-offer.html`, `upsell-bundle-tier-pills-offer.html`, `upsell-bundle-tier-cards-offer.html`, `upsell-single-offer.html` are deprecated one-line delegates to the shell (single maps `offer_features` / `offer_comparison` / `offer_steps` / `guarantee_note` onto the sections + closing card) so copied pages keep rendering. Catalog `surfaces[]` for apollo list the new set.
