...
- **Default (fallback)** → `data-next-default-property-key` collects one value and applies it to **every** line the active bundle writes. **Works on any selector, including a plain non-MV tier-swap** (olympus: 2x tier → one line carrying the default). The simple "one input, propagate everywhere" path — no slot picker needed.
