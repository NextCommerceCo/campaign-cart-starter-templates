...
| Template | Pages |
|----------|-------|
| apollo | /apollo/presell/ · /apollo/landing/ · /apollo/checkout/ · /apollo/upsell-bundle-stepper/ · /apollo/upsell-bundle-tier-pills/ · /apollo/upsell-bundle-tier-cards/ · /apollo/upsell-single/ · /apollo/upsell-vsl/ · /apollo/receipt/ |
| apollo-mv-single-step | /apollo-mv-single-step/presell/ · /apollo-mv-single-step/landing/ · /apollo-mv-single-step/checkout/ · /apollo-mv-single-step/variant-picker/ · /apollo-mv-single-step/upsell-mv/ · /apollo-mv-single-step/upsell-bundle-stepper/ · /apollo-mv-single-step/upsell-bundle-tier-pills/ · /apollo-mv-single-step/upsell-bundle-tier-cards/ · /apollo-mv-single-step/upsell-single/ · /apollo-mv-single-step/upsell-vsl/ · /apollo-mv-single-step/receipt/ |
| olympus | /olympus/presell/ · /olympus/landing/ · /olympus/checkout/ · /olympus/upsell-bundle-stepper/ · /olympus/upsell-bundle-tier-pills/ · /olympus/upsell-bundle-tier-cards/ · /olympus/upsell-single/ · /olympus/upsell-vsl/ · /olympus/receipt/ |
| olympus-mv-single-step | /olympus-mv-single-step/presell/ · /olympus-mv-single-step/landing/ · /olympus-mv-single-step/checkout/ · /olympus-mv-single-step/variant-picker/ · /olympus-mv-single-step/upsell-mv/ · /olympus-mv-single-step/upsell-bundle-stepper/ · /olympus-mv-single-step/upsell-bundle-tier-pills/ · /olympus-mv-single-step/upsell-bundle-tier-cards/ · /olympus-mv-single-step/upsell-single/ · /olympus-mv-single-step/upsell-vsl/ · /olympus-mv-single-step/receipt/ |
| olympus-mv-two-step | /olympus-mv-two-step/presell/ · /olympus-mv-two-step/landing/ · /olympus-mv-two-step/select/ · /olympus-mv-two-step/checkout/ · /olympus-mv-two-step/variant-picker/ · /olympus-mv-two-step/upsell-mv/ · /olympus-mv-two-step/upsell-bundle-stepper/ · /olympus-mv-two-step/upsell-bundle-tier-pills/ · /olympus-mv-two-step/upsell-bundle-tier-cards/ · /olympus-mv-two-step/upsell-single/ · /olympus-mv-two-step/upsell-vsl/ · /olympus-mv-two-step/receipt/ |
| demeter | /demeter/presell/ · /demeter/landing/ · /demeter/checkout/ · /demeter/upsell-bundle-stepper/ · /demeter/upsell-bundle-tier-pills/ · /demeter/upsell-bundle-tier-cards/ · /demeter/upsell-single/ · /demeter/upsell-vsl/ · /demeter/receipt/ |
| shop-single-step | /shop-single-step/presell/ · /shop-single-step/landing/ · /shop-single-step/checkout/ · /shop-single-step/upsell-bundle-stepper/ · /shop-single-step/upsell-bundle-tier-pills/ · /shop-single-step/upsell-bundle-tier-cards/ · /shop-single-step/upsell-single/ · /shop-single-step/upsell-vsl/ · /shop-single-step/receipt/ |
| shop-three-step | /shop-three-step/presell/ · /shop-three-step/landing/ · /shop-three-step/information/ · /shop-three-step/shipping/ · /shop-three-step/billing/ · /shop-three-step/upsell-bundle-stepper/ · /shop-three-step/upsell-bundle-tier-pills/ · /shop-three-step/upsell-bundle-tier-cards/ · /shop-three-step/upsell-single/ · /shop-three-step/upsell-vsl/ · /shop-three-step/receipt/ |
...
When adding a new `src/<slug>/` family, update these together or the **`lint-sdk` CI gate** fails. That job checks next-core and NEXT logo parity, generated shared assets, verification-evidence shape and coverage, the built SDK markup, and finally reports verification freshness as an informational warning. Freshness warnings self-heal, with no human in the loop, via the `refresh-template-verification` workflow (`.github/workflows/refresh-template-verification.yml`): after every green `lint:sdk` run on `main` with stale evidence it regenerates `template-verification.json`, pushes `bot/recertify-template-evidence`, **opens the re-certification PR itself, and arms auto-merge** so the PR lands once `lint-sdk` passes. It authenticates as the org-owned **`next-recertify-bot` GitHub App** (PR #164) — App-token PRs trigger CI normally, unlike the default `github.token`. So an ordinary recertification is invisible: the record of it is the merge in history, not a PR waiting in the queue. Both automated steps degrade rather than fail — if `gh pr create` is ever refused the run stays green and **comments on the source PR** (the one whose merge staled the evidence) with a compare link to open the PR by hand (direct pushes with no PR get the link in the run's step summary), and if the repo's *Allow auto-merge* setting is ever turned off the merge request is refused, one line is logged, and the PR waits for a human. Mechanical facts only — `campaigns_os_status` stays a human decision; manual path: `npm run refresh:template-verification`. History and trade-offs: issues #131/#163 (token strategy, closed) and PRs #139 → #164 → #162.
...
**Promoted checkout component styles** — Apollo, Apollo MV, and Olympus MV checkout UI (promo banner / countdown timer, checkout header, product heading, as-seen-in, reviews slider, guarantee, checkout reveal, Apollo bundle cards, MV selector layout) lives in the appended block at the end of `next-core.css`. Do **not** add per-page CSS links for these on Apollo/Apollo MV checkout — only CDN deps like Swiper and route-specific files (`variant-picker.css`, `exit-intent-popup.css`, landing/presell `tokens.css`) stay outside core.
...
**`apollo`** — **recommended default** single-step checkout: Apollo direct-response layout (trust-bar header, product heading, promo-banner / countdown-timer partials, as-seen-in, reviews slider, guarantee, optional checkout reveal) plus an include-driven tiered bundle selector with interchangeable card styles.
...
**`upsell-single`** (all eight commerce families) — single-offer upsell: one package, fixed price, accept or skip, no tiers. Composed from the shared upsell partial set (`upsell/offer.html` with `upsell_offer.type: single` + `upsell/body.html` + `upsell/closing-cta.html`; surface token `single`). Frontmatter: `upsell_offer` (+ optional `voucher` → hidden linked bundle selector, prices read `bundle.<selector_id>.*`), `offer_features`, `offer_comparison`, `offer_steps`, `guarantee_note`, `countdown_timer`, `announcement_text`, `pricing_mode` (`full_price` default — one row, per the shared commerce contract · `discounted`: struck compare + % badge + price · `compare_at`; every demo page opts into `discounted`). Demo copy is a protection plan; every string is an override. Drone-campaign families demo package 12 + `LIFE50`; MV families demo package 77 with no voucher.

### Composable upsell partials (all eight families)

Every upsell page in every family is assembled from **one interchangeable partial set** — canonical source `_shared/upsell/`, generated into every `src/<family>/_includes/upsell/` by `npm run sync:shared` and guarded by `lint:shared` (byte-identical, no per-family exceptions), selected and filled from frontmatter. A bundle upsell, the single-offer upsell and the long-form VSL upsell are the same includes with different data (`upsell-bundle-stepper.html` vs `upsell-vsl.html` share `upsell_offer.type: stepper`). Page order:

```
upsell/announcement.html   announcement_text (+ announcement_variant: primary | danger | success | dark | light)
upsell/header-bar.html     variant: steps (default) | logo
upsell/hero.html           upsell_hero.variant: band (title · offer · suboffer) | card (short band + white card: heading · text · countdown_timer) | plain (centered h1 · text on the surface colour — pairs with upsell_layout.style: box)
countdown-timer.html       (band pages include it directly; the hero card embeds it)
upsell/offer.html          upsell_offer.type: tier-cards | tier-pills | stepper | single | mv   ← the shell (include arg offer_type= overrides)
  ├ upsell/offer-context.html   resolves offer / package_id / selector_id / price paths / tiers ONCE (assign shares scope; upsell/offer-tier.html resolves one tier per loop index)
  ├ display-only selector       tier-cards · tier-pills · mv → bundle.upsell-bundle-1x.* pinned for hero copy — OUTSIDE the offer
  │                             single + voucher → hidden linked selector (data-next-bundle-selector-id on the wrapper) — OUTSIDE
  └ [data-next-upsell="offer"]  (the ONLY SDK offer wrapper on the page)
      ├ upsell/offer-selector.html   hidden state machine (pills / stepper card w/ qty attrs / mv configurable cards + slot <template>) — FIRST in-offer selector
      ├ content column: upsell-offer-heading (pretitle · title · rating · optional heading_prices strip) → upsell-video (own include, when upsell_video is set) → upsell-offer-bullets → upsell-offer-controls (the ONLY per-type piece) → upsell-offer-prices (pricing_mode) → upsell-cta (accept · cta_note · upsell/ships-info [ships_info { icon, flag, text }] · payment-logos.html [shared with checkout — one <img data-payment-logo=code> per method; initPaymentLogos() in js/payment-logos.js (load on every page that includes it) reveals the ones in next.getCampaignData().available_payment_methods + available_express_payment_methods on next:initialized (supported SDK call only — no storage reads; the campaign API round-trip is ~200ms so cards-only-until-init is not visible). Cards: the platform exposes one `bankcard` code and the merchant's gateway accepted_card_types list never reaches the campaign payload, so bankcard shows visa/mastercard/american_express/discover by default and diners_club/jcb/maestro/dankort are opt-in via payment_flags.show_<brand> (platform codes); any brand can be forced off; payment_flags.show_<code> true|false is a per-method override; style: flat = legacy sprite]) → upsell-guarantees
      ├ media column:   upsell-offer-media (upsell_layout.media: carousel | image | grid | video | none — the swiper area; video reads media_video, a second object with the upsell_video contract, so the content-column VSL and a media-column video can coexist; carousel knobs swiper_aspect / swiper_thumb_aspect / swiper_fit / swiper_thumbs_per_view) + payment logos
      └ upsell/decline.html
upsell/body.html           upsell_body[]: { type: reviews | features | comparison | steps } → upsell-reviews / -features / -comparison / -steps
upsell/closing-cta.html  closing_cta.variant: cta | card (+ title, note, background: surface | tint | none, show_guarantees) — accept/skip are data-upsell-proxy buttons that add whatever was selected in the main offer; no quantity control here by design
footer.html
```

Rules that keep the SDK happy:
- **One offer wrapper per page.** `UpsellEnhancer` resolves the bundle to add from the *first* `[data-next-bundle-selector]` inside its own `[data-next-upsell="offer"]`, so the closing CTA never gets a second wrapper. `upsell/closing-cta.html` renders `data-upsell-proxy="add|skip"` and `initUpsellProxyActions()` (`upsells.js`, self-initialising) clicks the real in-offer action. Display-only / voucher selectors stay outside the wrapper; the hidden state selector is rendered first inside it.
- **Switching `upsell_offer.type` alone always yields a working selector.** Per-type inputs: `single` → `package_id` (+ `voucher`); `stepper` → `package_id` (+ `items_json`, `vouchers_json`, `default/min/max_quantity`); `tier-cards` / `tier-pills` → `package_id` + `upsell_bundle_tiers[]`; `mv` → `package_id` + `upsell_mv_tiers[]` + `upsells-mv.js`. When the tiers array is absent, `upsell/offer-context.html` generates `offer.tier_count` (default 3) tiers from `package_id` — ids `<selector_id>-<n>x`, items `{packageId, quantity: n}`, `offer.vouchers_json` (default `["UP50"]`) on every tier, labels "Buy n" — via the shared `upsell/offer-tier.html` resolver that the selector and controls loops both call. Declare `upsell_bundle_tiers` only when tiers need distinct vouchers, labels or hints.
- **`upsell_layout`** — `style: card` (default: the bundle look — band overlap, shadowed container, content card with bullets/ships-info/decline) or `style: box` (the upsell-single look — bordered `upsell-container--box` in a surface section, image left `cc-medium`, centered `cc-fluid` copy, rating-first heading with the % badge, inline `Yours Today` price, `cc-sm` button with the live price, no bullets/ships-info/decline unless `show_bullets` / `show_decline`); `media_width: narrow | medium | wide` (box style column width), `media_align: center | top | sticky` (sticky needs `boxed: false`; the shadowed container clips it), `decline_position: below | column` (decline under the whole offer, or inside the content column after the CTA / guarantees), `boxed` (false = no card/shadow; pair with `upsell_hero.variant: card`), `media`, `media_position` (`right` default = content left; `left` adds `cc-media-left`), `show_bullets`, `show_ships_info`, `show_payment_logos`.
- **`upsell_video`** — native `<video>` (`src`, `poster`, `title`, `autoplay` default true → `autoplay muted`, `loop`, `controls`, `preload` default `metadata` when autoplaying / `none` when `autoplay: false` so only the poster loads until play, `id`); no Plyr / custom controls / iOS sound-unlock JS in the template. The frame reserves 16:9 in CSS (no layout shift); browsers ignore `loading="lazy"` on video, `preload` is the lever. **Custom player:** the element carries `data-upsell-video` (+ your `id`) — set `controls: false`, add the player CSS/JS to the page `styles:`/`scripts:` (Plyr 3.7.8 on cdn.jsdelivr.net verified) and your own small `js/` file that targets `[data-upsell-video]`; the partial stays generic. its own include, always under the heading (the fluff upsell-4/5 placement), independent of `upsell_layout.media`. NEXT-hosted media → `cdn.cachebucket.com`.
- **`pricing_mode`** lives in `upsell/offer-prices.html`: `discounted` (default; Retail struck | Offer + % OFF) · `compare_at` · `full_price` · `unit_total` · `unit_only`. The partial default is `full_price` (one row) per the shared commerce contract `template-brand-contract.shared-commerce.v0` `pricing_surfaces.upsell.default_mode`, so a campaign that says nothing gets the QA-safe single row; every demo page opts into `discounted` explicitly. `bundle.<sel>.unitPrice` is not a reliable remote path — prefer `discounted` on bundle offers. `retail_shipping` / `offer_shipping` `""` hide those lines. When `upsell_offer.heading_prices: true` the rows are skipped (the heading strip already shows the price) unless `price_rows: true`.
- **Copy keys are HTML strings**: `upsell_hero.*`, `upsell_offer.title/pretitle/cta_note`, body section titles — write `data-next-display` spans in frontmatter (LiquidJS does not escape output). Page files then contain no SDK attributes, which is what `lint:sdk` source mode wants.
- **MV**: `upsell_offer.type: mv` (the three MV families' `upsell-mv.html`) — `upsell/offer-selector.html` renders the hidden configurable cards + slot `<template>` (with `slot_personalization`), `upsell/offer-controls.html` the qty pills, `order_personalization` field and `#bundle-slots-stage`; `upsells-mv.js` only styles the SDK-injected slot selects now (the qty toggle moved to the shared `upsells.js` — load both). Browser-verified 2026-09-25 on apollo-mv-single-step: slots 1 → 3 on the qty pill, per-slot property inputs cloned, one SDK offer.
- Core CSS for all of this is the appended `Composable upsell partials` block in `next-core.css` (tier-card grid promoted from the old page-level `<style>`, `.cc-plain`, `.cc-media-left`, `.upsell-video*`, `.upsell-guarantee*`, `.upsell-header--card` / `.upsell-header-card*`, `.upsell-review*`, `.announcement.cc-danger/-success/-dark/-light`, `.upsell-header-bar--logo`, `.upsell-closing*`). Synced to all eight families.

### Upsell frontmatter reference (apollo composable set)
...
| `upsell_offer` | `type` tier-cards · tier-pills · stepper · single · mv; `package_id`; `selector_id`, `display_selector_id`, `display_bundle_id`; `voucher` (single); `bundle_id`, `items_json`, `vouchers_json`, `default_quantity`, `min_quantity`, `max_quantity` (stepper); `tier_count` (generated tiers); `variant_labels` (mv); `pricing_mode` full_price (default, one row — shared contract) · discounted · compare_at · unit_total · unit_only; `price_rows` (force rows when `heading_prices`); copy: `pretitle`, `title`, `show_rating`, `rating_text`, `heading_prices`, `quantity_label`, `retail_label`, `price_label`, `unit_label`, `total_label`, `retail_shipping`, `offer_shipping`, `urgency_text`, `accept_text`, `accept_show_price`, `cta_note`, `social_proof`, `details_hint`, `ships_info_text` (legacy), `decline_text`, `closing_title`, `image`, `image_alt`, `image_display` (e.g. `package.image` — SDK swaps in the live package image for `media: image`) |
...
| `payment_flags` | per-method / per-brand override only: `show_<code>` true/false for methods (`paypal`, `apple_pay`, `google_pay`, `klarna`, `affirm`, `link`, `twint`, `bancontact`, `ideal`, `sepa_debit`) and card brands as `show_<brand>` with the platform codes (`show_visa`, `show_mastercard`, `show_american_express`, `show_discover` default on; `show_diners_club`, `show_jcb`, `show_maestro`, `show_dankort` opt-in), `secure_text`, `style: flat`; otherwise logos resolve from the campaign's payment methods |
...
**`countdown-timer.html`** (all eight commerce families) — slim urgency bar, **not** the `<promo-timer>` web component the non-apollo checkouts still use. Two modes from `countdown_timer` data: `duration` (seconds) → SDK `data-next-timer`, persisted per `persistence_id` (upsell pages); `start` (`"MM:SS"`) → legacy `[data-next-element="timer"]` driven by `checkout.js` (checkout). Renders nothing when absent. CSS classes are `.countdown-timer*` in core.
...
- Optional **GTM / Meta Pixel** in reference templates: injected from `campaign.gtm_id` / `campaign.fb_pixel_id` when Liquid `environment != "development"` **and** the value is **present and non-empty** (hardened guard `{% if campaign.gtm_id and campaign.gtm_id != "" %}` / `{% if campaign.fb_pixel_id and campaign.fb_pixel_id != "" %}` — absent OR empty = off). Use **`""`** (or omit the key) in `campaigns.json` to disable layout injection; **placeholders like `GTM-XXXXXXX` still load snippets** on non-dev builds (not “off”). Do **not** use bare `{% if campaign.gtm_id %}` — Liquid can treat `""` as truthy. The snippets themselves live in shared partials (see next bullet), not inline in each layout.
...
| `data-next-timer` + `data-duration="<seconds>"` | **SDK-native countdown** (`TimerEnhancer`, source-verified v0.4.38). Optional `data-persistence-id` (default `default-timer`; start time persists in `localStorage` so a reload does not restart it — give each offer its own id) and `data-format` (default `mm:ss`). Inner `[data-next-timer-display]` receives the formatted time; `[data-next-timer-expired][data-persistence-id=…]` anywhere on the page is revealed at zero. No page JS needed. Used by `countdown-timer.html` in `duration` mode; distinct from the legacy `data-next-element="timer"` handler in `checkout.js`. |
...
- Surfaces + partial defaults (per contract): upsell offers (`upsell/offer-prices.html` and `upsell/offer-heading.html`, reached by every offer type through the shared `upsell/offer.html` shell — the same default in ALL families, since the Demeter contract only extends the shared commerce contract) default **full_price** — exactly one visible price row adjacent to the accept control; `bump-check01.html` defaults **full_price** (one `unitPrice`/ea row); `editorial-tier-selector.html` defaults **discounted** (current compare-unit-total render), with per-card override via `bundles[].pricing_mode`.
...
| Template | JS Files |
|----------|----------|
| demeter | checkout.js, checkout-demeter.js, upsells.js, payment-logos.js, promo-banner.js, promo-timer.js |
| olympus | checkout.js, checkout-olympus.js, upsells.js, payment-logos.js, promo-banner.js, promo-timer.js |
| apollo | checkout.js, checkout-apollo.js, upsells.js, payment-logos.js |
| apollo-mv-single-step | checkout.js, checkout-apollo-mv-full.js, upsells-mv.js, upsells.js, payment-logos.js, variant-picker.js, variant-picker-fixture.js |
| olympus-mv-single-step | checkout.js, checkout-olympus-mv-full.js, upsells-mv.js, upsells.js, payment-logos.js, promo-banner.js, promo-timer.js, variant-picker.js, variant-picker-fixture.js |
| olympus-mv-two-step | checkout.js, checkout-olympus-mv-full.js, checkout-olympus-mv-selection.js, upsells-mv.js, upsells.js, payment-logos.js, promo-banner.js, promo-timer.js, variant-picker.js, variant-picker-fixture.js |
| shop-single-step | checkout.js, upsells.js, payment-logos.js, promo-banner.js, promo-timer.js |
| shop-three-step | checkout.js, checkout-shop-three-billing.js, checkout-shop-three-shipping.js, upsells.js, payment-logos.js, promo-banner.js, promo-timer.js |
