...
**Model: shared default + per-slot override** — a global fallback with optional per-line flexibility. Engineering states it as "default everywhere + override per slot"; **browser-verified reality (2026-06-23) below — the override half is gated on the slot-rendering setup and has a same-package reliability bug. Pending eng confirmation that this is intended.**
- **Default (fallback)** → `data-next-default-property-key` collects one value and applies it to **every** line the active bundle writes. **Verified on any selector, including a plain non-MV tier-swap** (olympus: 2x tier → one line carrying the default). The simple "one input, propagate everywhere" path.
- **Override** → `data-next-property-key` on an **individual slot** replaces the default for that line. **Needs the slot-rendering selector** (`data-next-bundle-slots-for` + slot `<template>`); a plain tier-swap renders **no slots**, so there is **no override there** — default only. The slot structure does **not** require a variant product: a standalone product works by listing its packageId N times with `configurable:false` (verified: standalone pkg 76 ×3 → 3 personalizable lines).
  - **Verified failure:** eng's literal "set a default, override only some slots, leave the rest on the default" flow **does not work for repeats of the same package** — the lone override is dropped and the cart keeps the default for all units (`setItemProperties` is packageId-keyed; it can't carve one unit out of a merged same-package line). Per-line values are reliable **only when every slot carries its own explicit value** (e.g. `[DAD, MOM, DAD]` → 2×DAD + 1×MOM, verified).
...
2. **Order-wide — `data-next-default-property-key` (any input, outside slots).** The shared default above (`mergeWithDefaults` on cart swap; per-slot values override). Toggle: `order_personalization` frontmatter → `personalization-field.html` partial. Shipped enabled on `olympus/checkout.html`.
