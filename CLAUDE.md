...
**Intended model (per engineering): shared default + per-slot override** — a global fallback with optional per-line flexibility.
- **Default (fallback)** → `data-next-default-property-key` collects one value and applies it to **every** line the active bundle writes — **works on non-MV bundles too**. The simple "one input, propagate everywhere" path (regardless of quantity or how many package ids).
- **Override** → `data-next-property-key` on an **individual slot** replaces the shared default **for that line only**.
