...
| `data-next-product-sync="<productId>"` | Order-bump quantity sync, matched by **productId** (SDK 0.4.25+). Covers every variant of a configurable/MV product with one id — the correct attribute for MV bumps. |
...
- **Current fix (SDK 0.4.25+, applied on `olympus-mv-single-step` + `olympus-mv-two-step` `bump-check01.html`):** use **`data-next-product-sync="<productId>"`** — matched on `item.productId`, which every variant of the same product shares, so the SDK sums quantity across all variants with one id. Find the value in the campaign API response under `packages[].product_id` (identical for every variant of a product). Update the id when cloning to another product.
