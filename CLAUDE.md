...
Edit the file in `_shared/analytics/`, then `npm run sync:shared` (writes the per-family copies with a `GENERATED …` header) and `npm run lint:shared` (CI drift gate — same discipline as `lint:next-core`). The `js/` files are kept **byte-identical to `analytics-tracking-docs/examples/`** so re-syncing from the upstream reference is a straight copy. `_shared/analytics/README.md` has the vendor→id table.

**Off by default; toggle = id presence.** The capability is inert until a campaign sets the vendor's id in `campaigns.json` (`ga4_id`, `axon_event_key`, `taboola_account_id`, `triplewhale_name`, `tiktok_pixel_id`, `northbeam_client_id`, `snap_pixel_id`, `pinterest_tag_id`; plus `gtm_id`/`fb_pixel_id`). The shared `next-forwarder-core.js` loads once when **any** id is set; each adapter loads only when its own id is set. Per-campaign `*_allowed_events`/`*_blocked_events` tune which `dl_*` events forward; PII/identity (`*_enabled` flags) is off unless set and then consent-gated on `accepts_marketing`. Authoring intent lives upstream in the campaigns-os `AnalyticsContract` (`enabled` + id), which compiles to these `campaigns.json` ids — the template only ever reads the id (one source of truth, no second runtime flag). Adapter mapping is regression-tested by `analytics-tracking-docs/examples/_harness/`.
