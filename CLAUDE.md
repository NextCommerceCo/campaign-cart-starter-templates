...
When adding a new `src/<slug>/` family, update these together or the **`lint-sdk` CI gate** fails. That job checks next-core and NEXT logo parity, generated shared assets, verification-evidence shape and coverage, the built SDK markup, and finally reports verification freshness as an informational warning. Freshness warnings self-heal via the `refresh-template-verification` workflow (`.github/workflows/refresh-template-verification.yml`): after every green `lint:sdk` run on `main` with stale evidence it regenerates `template-verification.json` and pushes `bot/recertify-template-evidence`. This repo doesn't allow Actions to open PRs, so instead of failing red the run stays green and **comments on the source PR** (the one whose merge staled the evidence) with a compare link to open the re-certification PR from that branch (a human-opened PR runs CI normally — no close/reopen); direct pushes with no PR get the link in the run's step summary. Adding a `RECERTIFY_TOKEN` fine-grained PAT makes it open the PR itself. Mechanical facts only — `campaigns_os_status` stays a human decision; manual path: `npm run refresh:template-verification`; trade-offs in issue #131.
