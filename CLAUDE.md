...
Templates follow SDK **0.4.x** patterns (`olympus`, `apollo`, `limos`, `demeter`, `olympus-mv-single-step`, `olympus-mv-two-step`, `shop-single-step`, `shop-three-step` today).
...
- Because the whole bump card is the PackageToggle click target, `checkout-olympus-mv-full.js` adds `guardBumpPersonalization()` — it stops click/keydown inside `.bump__personalization` from toggling the bump (browser-verified: a field click does not reach the card; a title click does). The `bump-check03` partial is **unsynced by default** (one standalone add-on product) but supports **opt-in quantity-sync** via a `package_sync` include arg / `order_bump.check03.sync_quantity` frontmatter (renders `data-next-package-sync`, packageId match). The olympus-mv reference leaves it unsynced; **apollo's checkout sets `sync_quantity: 1`** so the add-on scales with the 1x/2x/3x main bundle selection. For an MV/configurable main product use `data-next-product-sync` (product_id match) instead — see the MV order-bump-sync note above.
...
| apollo | checkout.js, checkout-apollo.js, upsells.js, promo-banner.js, promo-timer.js |
| apollo-mv-single-step | checkout.js, checkout-apollo-mv-full.js, upsells-mv.js, upsells.js, promo-banner.js, promo-timer.js, variant-picker.js, variant-picker-fixture.js |
