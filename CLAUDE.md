...
- **Default (fallback)** → `data-next-default-property-key` collects one value and applies it to **every** line the active bundle writes. **Works on any selector, including a plain non-MV tier-swap** (verified: a tier-swap 2x → one line carrying the default). The simple "one input, propagate everywhere" path — no slot picker needed. (Documented for completeness, but **not shipped as a live example on the non-MV checkouts** — per the decision below, the realistic use case is the slot picker.)
...
2. **Order-wide — `data-next-default-property-key` (any input, outside slots).** The shared default above (`mergeWithDefaults` on cart swap; per-slot values override). Toggle: `order_personalization` frontmatter → `personalization-field.html` partial. **Shipped on `olympus-mv-single-step/checkout.html` only**, where it pairs with the per-slot inputs as the override-able default (same `property_key`). Deliberately **removed from `olympus`/non-MV checkouts** — a shared-value-only field has little real use; the slot picker is the expected pattern. The partial + `order_personalization` contract remain available to drop onto any checkout.
