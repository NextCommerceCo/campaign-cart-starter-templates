...
const CONFIG = {
  ...
  // Free T-Shirt Configuration (Slot 4 for Tier 3)
  freeTShirt: {
    enabled: true,
    productId: null, // Will use base product with special package mappings
    packageMappings: {
      // Map color/size combinations to free t-shirt package IDs
      'White-Small': 55,
      'Gray-Small': 56,
      'Black-Small': 57,
      'White-Medium': 58,
...
class TierController {
  ...
  async init() {
    ...
    if (showSelection && hasCartItems) {
      ...
      requestAnimationFrame(() => {
        ...
        // Update cart with default selections
        setTimeout(() => {
          this._updateCart();
        }, 100);
      });
  ...
  _setDefaultsWithoutCart() {
    ...
    // Determine max slot (includes slot 4 for tier 3)
    const maxSlot = this.currentTier === 3 && CONFIG.freeTShirt.enabled ? 4 : this.currentTier;
  ...
  _initStateFromCart(cartData) {
    ...
    requestAnimationFrame(() => {
      ...
      // Update cart to sync with restored state
      setTimeout(() => {
        this._updateCart();
      }, 100);
    });
  ...
  async selectTier(tier, skipCartUpdate = false) {
    ...
    // Clear slot 4 selections when switching away from tier 3
    if (prev === 3 && tier !== 3) {
      this.selectedVariants.delete(4);
    }
    ...
    // Update cart in real-time (if not skipped)
    if (!skipCartUpdate) {
  ...
  _copySelectionsToNewSlots(fromTier, toTier) {
    ...
    // Determine max slot (includes slot 4 for tier 3)
    const maxSlot = toTier === 3 && CONFIG.freeTShirt.enabled ? 4 : toTier;

    for (let i = fromTier + 1; i <= maxSlot; i++) {
      // Skip regular slots beyond tier
      if (i > toTier && i !== 4) continue;
  ...
  _updateSlots(tier) {
    ...
    // Determine max slots (tier 3 gets 4 slots including free t-shirt)
    const maxSlots = tier === 3 && CONFIG.freeTShirt.enabled ? 4 : tier;

    // Hide slots beyond maxSlots
    container.querySelectorAll('[next-tier-slot]').forEach(slot => {
      const num = +slot.getAttribute('next-tier-slot');
      slot.style.display = num > maxSlots ? 'none' : 'flex';
      slot.classList.toggle('active', num <= maxSlots);
    });

    // Create missing slots (including slot 4 for tier 3)
    for (let i = 2; i <= maxSlots; i++) {
      if (!container.querySelector(`[next-tier-slot="${i}"]`) && template) {
        ...
        // Mark slot 4 as free t-shirt slot
        if (i === 4) {
          slot.setAttribute('data-free-slot', 'true');
        }
    ...
    // Ensure slot 4 is marked correctly if it already exists
    const slot4 = container.querySelector('[next-tier-slot="4"]');
    if (slot4 && tier === 3) {
      slot4.setAttribute('data-free-slot', 'true');
    }
  ...
  _updateCartDebounced() {
    ...
    // Set new timer - wait 500ms after last change before updating cart
  ...
  async _updateCart() {
    // Only update if we have at least one complete selection
    const hasCompleteSelection = Array.from(this.selectedVariants.values()).some(
      v => v?.color && v?.size
    );

    if (!hasCompleteSelection) {
      // Clear cart if no complete selections
      try {
        await window.next.swapCart([]);
      } catch (err) {
    ...
    try {
      ...
      // Build cart items for slots 1-3 (paid items)
      ...
      // Add free t-shirt (slot 4) if tier 3 and selections are complete
      if (this.currentTier === 3 && CONFIG.freeTShirt.enabled) {
        const v = this.selectedVariants.get(4);
        if (v?.color && v?.size) {
          // Map color/size to free t-shirt package ID
          const key = `${v.color}-${v.size}`;
          const freePackageId = CONFIG.freeTShirt.packageMappings[key];

          if (freePackageId) {
            items.push({ packageId: freePackageId, quantity: 1 });
          }
        }
      }

      // Swap cart with new items
      if (items.length > 0) {
  ...
  _populateDropdowns(slot, slotNum) {
    // For free t-shirt slot (slot 4), use base product for options
    const isFreeSlot = slotNum === 4 && slot.getAttribute('data-free-slot') === 'true';
    const pid = isFreeSlot ? this.baseProductId : ((slotNum === 1 && this.baseProductId) || this.productId);
  ...
  _updatePrices() {
    ...
    // Also update slot 4 if tier 3
    if (this.currentTier === 3 && CONFIG.freeTShirt.enabled) {
      this._updateSlotPrice(4);
    }
  ...
  _updateSlotPrice(slotNum) {
    ...
    // Handle free t-shirt slot (slot 4)
    const isFreeSlot = slotNum === 4 && slot.getAttribute('data-free-slot') === 'true';
    if (isFreeSlot) {
      const v = this.selectedVariants.get(slotNum);
      if (!v?.color || !v?.size) {
      ...
      if (regEl) regEl.textContent = '$29.95';

      const pctEl = slot.querySelector('[data-option="savingPct"]');
      if (pctEl) pctEl.textContent = 'FREE';

      const priceContainer = slot.querySelector('.os-card__price.os--current');
      if (priceContainer) {
        priceContainer.innerHTML = '<span data-option="price">$0.00</span>/ea';
      }
      return;
    }
  ...
  _isComplete() {
    // Check slots 1-3 (or 1-2, or 1 depending on tier)
    for (let i = 1; i <= this.currentTier; i++) {
      const v = this.selectedVariants.get(i);
      if (!v?.color || !v?.size) return false;
    }

    // Also check slot 4 if tier 3 (free t-shirt)
    if (this.currentTier === 3 && CONFIG.freeTShirt.enabled) {
      const v = this.selectedVariants.get(4);
      if (!v?.color || !v?.size) return false;
    }
  ...
  _updateAllPrices() {
    // Update dropdowns and re-select variants for slots 1-3
    for (let i = 1; i <= this.currentTier; i++) {
    ...
    // Also update slot 4 if tier 3
    if (this.currentTier === 3 && CONFIG.freeTShirt.enabled) {
      const slot = document.querySelector(`[next-tier-slot="4"]`);
      if (slot) {
        this._populateDropdowns(slot, 4);

        const variants = this.selectedVariants.get(4);
        if (variants?.color && variants?.size) {
          this._updateSlot(4, variants);
        }
      }
    }
  ...
  handleVerifyClick() {
    if (!this._isComplete()) {
      ...
      // Also check slot 4 if tier 3
      if (this.currentTier === 3 && CONFIG.freeTShirt.enabled) {
        const v = this.selectedVariants.get(4);
        if (!v?.color || !v?.size) {
          document.querySelector(`[next-tier-slot="4"]`)?.scrollIntoView({
            behavior: 'smooth',
            block: 'center'
          });
        }
  ...
  _isVariantOOS(slotNum, variant) {
    ...
    // For free t-shirt slot, check using free package IDs
    if (slotNum === 4) {
      const key = `${variant.color}-${variant.size}`;
      const freePackageId = CONFIG.freeTShirt.packageMappings[key];
      if (!freePackageId) return true; // If no mapping, consider OOS

      const pkg = window.next.getPackage(freePackageId);
