...
- **Templates / where:** Any page with `[data-next-cart-summary]` template using `cart.hasCoupon` + `cart.discountCode`; reference comments in [`olympus/checkout.html`](../campaign-kit-templates/src/olympus/checkout.html) and [`olympus-mv-single-step/checkout.html`](../campaign-kit-templates/src/olympus-mv-single-step/checkout.html).
