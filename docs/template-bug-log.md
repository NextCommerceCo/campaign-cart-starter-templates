...
| MV upsell: exit/checkout **coupon bleeds** into upsell bundle display when same **`packageId`** shared on checkout + upsell — **display-only**; actual charge correct | *(not in migration §)* | **BS-018** (`medium`) |
...
- Workaround (templates):
  - Use **`data-summary-voucher-discounts`** + **`{discount.name}`** (and amount) in the summary template, or small **custom JS**. **`data-next-show=”cart.hasCoupon”`** is safe to use for conditional visibility.
- Cross-ref:
  - Migration **§ Known #10**.
...
## BS-018 - MV upsell: exit/checkout coupon bleeds into upsell bundle display pricing when same `packageId` is reused

- Status: `open`
- Severity: `medium`
- Date logged: `2026-04-06`
- **Templates / where:** Any MV upsell using `data-next-bundle-selector` with a `packageId` shared with the checkout bundle selector — primary: [`olympus-mv-single-step/upsell-mv.html`](../campaign-kit-templates/src/olympus-mv-single-step/upsell-mv.html).
- **Confirmed test case:** packageId 1, base $39.98. Checkout offer 50% → $19.99. `EXIT10` applied → $17.99. Upsell `UP80TSHIRT` (80% off) should show **$8.00 / 80%** — instead showed **$7.20 / 82%** (`EXIT10` stacked on top of upsell voucher in `calculateBundlePrice` display).
- **Display-only (no-variant-change path):** Actual charge and receipt are correct. `AddUpsellLine` submission uses only the declared `data-next-bundle-vouchers` and prices correctly. Only `calculateBundlePrice` display tokens are affected — but see variant-change path below.
- **UX risk:** Shopper sees a deeper discount than they were offered; receipt shows the real (higher) price — potential bait-and-switch perception.
- **Root cause — confirmed (2026-04-07):** Two compounding mechanisms:
  1. **Session coupon bleed:** SDK carries applied checkout coupons into `calculateBundlePrice` on the upsell page. Display tokens (`bundle.*.price`, `bundle.*.discountPercentage`) re-calculate with the session coupon stacked on the upsell voucher.
  2. **Variant resolver jumps package families:** `BundleSelectorEnhancer` builds its resolution pool as `allPackages.filter(p => p.product_id === currentPkg.product_id)`, then does `.find(...)` — first match wins. If checkout packages (e.g. 1–9) and upsell packages (e.g. 66–74) share the same `product_id` and identical variant attribute tuples (same size/color combinations), a variant change on the upsell slot can resolve to a checkout package (e.g. 2) instead of the upsell sibling (e.g. 67). The resolved `slot.activePackageId` is what drives both `calculateBundlePrice` (display) **and** `_getSelectedBundleItems` (accept/charge). If the upsell voucher is not scoped to that resolved id, it fails silently and the session coupon is the only discount — charge is wrong, not just display.
- **Why "scope voucher to 66–74" failed:** Resolved package was 2 (from checkout family), not 67. Voucher scoped to 66–74 didn't apply. Session `EXIT10` remained and became the only discount.
- **Why "scope voucher to 1–9" worked:** Resolver landed on 1–9 anyway; voucher eligibility matched the resolved id.
