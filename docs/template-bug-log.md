...
| Checkout **phone** field (`data-next-checkout-field="phone"`) — excessive **`padding-left`** (~52px) / intl-tel layout; older Campaign Cart CSS fix removed in newer SDK | `next-core.css` workaround in all 6 templates (remove on SDK fix) | **BS-016** (`workaround applied`) |
...
| **`data-next-include-shipping`** on **`[data-next-bundle-selector]`** — read/stored on price context but **not** passed to **`calculateBundlePrice`** (unlike **PackageSelector**) — **no-op** today | *(SDK parity)* | **BS-020** (`low`) |
...
- Status: `workaround applied` (SDK fix pending)
- Severity: `medium`
- Date logged: `2026-03-31`
...
- **Workaround applied (all templates):** Added to **`next-core.css`** in all 6 templates (`olympus`, `limos`, `demeter`, `olympus-mv-single-step`, `shop-single-step`, `shop-three-step`) — remove once SDK ships a fix:
  ```css
  ...
  .iti:not(.iti--allow-dropdown) input[type=tel] { padding-left: 12px !important; }
  .iti:not(.iti--allow-dropdown) input,
  .iti:not(.iti--allow-dropdown) input[type=text],
  .iti:not(.iti--allow-dropdown) input[type=tel] { padding-right: 52px !important; }
  ```
- Cross-ref:
...
## BS-020 - `data-next-include-shipping` on `[data-next-bundle-selector]` is a no-op (not passed to `calculateBundlePrice`)

- Status: `open`
- Severity: `low`
- Date logged: `2026-04-17`
- **Templates / where:** Any checkout with **`data-next-bundle-selector`** and **`data-next-include-shipping`**. Reference templates that set the attribute today: [`olympus/checkout.html`](../campaign-kit-templates/src/olympus/checkout.html), [`olympus-mv-single-step/checkout.html`](../campaign-kit-templates/src/olympus-mv-single-step/checkout.html) (for documented / future parity — **no effect** until SDK wires it).
- **Nature:** The attribute is **read** and stored on the bundle **price context**, but the bundle price helper **does not pass it through** to **`calculateBundlePrice`** the way **`PackageSelector`** does for the legacy path — behavior is **unchanged** whether the attribute is present, absent, or toggled.
- **Expected (once fixed):** Tier / bundle display totals respect **include shipping** the same as package selector when the flag is set.
- **Actual:** **No behavioral difference** — safe to **omit** on bundle selectors until core implements parity.
- **Workaround:** Do not rely on this flag for bundle card totals; derive shipping-inclusive presentation from cart summary / other live totals if required, or accept that bundle-tier headline numbers exclude this flag’s intent until SDK fix.
- **Next action:** SDK — thread **`includeShipping`** (or equivalent) from bundle price context into **`calculateBundlePrice`** for bundle selector the same as package selector.
- **Shareable index:** [Campaign issues overview](campaign-issues-overview.md) — open **#17**.
