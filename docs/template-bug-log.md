...
- **Display-only:** Actual charge and receipt are correct. `AddUpsellLine` submission uses only the declared `data-next-bundle-vouchers` and prices correctly. Only `calculateBundlePrice` display tokens are affected.
...
- **Root cause (hypothesis):** SDK carries the applied voucher/coupon state across page load into the upsell session. Display tokens (`bundle.*.price`, `bundle.*.discountPercentage`) re-calculate using the active session coupon.
- **Workaround:**
  - Use a separate `packageId` on the upsell scoped to a different offer in the Campaigns app so checkout coupons do not apply to it.
  - Investigate whether the session coupon can be cleared before display tokens resolve on upsell page load.
