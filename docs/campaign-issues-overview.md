...
| 3   | **`data-next-shipping-id` on bundle cards (BS-011, Known #3):** `data-next-shipping-id` on `data-next-bundle-card` now works — **fixed in SDK 0.4.12**. `calculateTotals` was previously hardcoded to shipping method 1; now uses the selected method, so summary shipping line and grand total update correctly on bundle tier change. **Note:** package swap selector path (`data-next-selector-card` + swap mode) still unreliable declaratively — JS mitigation (`next.setShippingMethod(refId)`) remains the workaround for that path. | **Bundle** checkouts using `data-next-shipping-id` on tier cards. | Medium | Fixed |
| 7   | **Cart summary (BS-012):** Amount column strikethrough showed per-unit retail instead of full-line retail on multi-qty lines. **Fixed in SDK 0.4.11:** `{item.originalPrice}` is now a line total (qty × original unit price). Templates updated: `data-summary-lines` template migrated from deprecated `{line.*}` → `{item.*}` namespace (`{line.*}` rendered silently blank); Amount strikethrough now uses `{item.originalPrice}`, `/ea` subtitle uses `{item.originalUnitPrice}`. | **All** campaigns using `[data-next-cart-summary]` with a compare/strikethrough column (reference `olympus`, `olympus-mv-single-step`). | High | Fixed |
| 8   | **`{line.*}` deprecated in `data-summary-lines`:** All `{line.*}` tokens inside `data-summary-lines` `<template>` rendered silently blank — no console warning. Correct namespace is `{item.*}` (SDK 0.4.11+). Reference templates updated; `docs/bundle-display-cart-cheatsheet.md` section 4 rewritten. | **All** campaigns using `data-summary-lines` line row templates. | High | Fixed |
