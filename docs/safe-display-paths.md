...
| `cart.currency` | ⚠️ Broken for **`data-next-display`** inside `[data-next-cart-summary]` — `CartDisplayEnhancer` reads from cart store `currency` field, which is only set after `setLastCurrency` runs (not on cached campaign load path); node stays empty. **Use `{currency}` template token** instead (same resolution chain as `{total}`). Never hardcode `"USD"` — it flashes before being overwritten. |
