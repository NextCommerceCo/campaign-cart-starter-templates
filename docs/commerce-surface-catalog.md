...
Machine-readable companion: [`commerce-surface-catalog.json`](./commerce-surface-catalog.json). The JSON catalog now includes an `agentContract` block for each first-class checkout family plus `sharedFrontmatterVocabulary` for cross-family fields such as `packages`, `shipping_methods`, `bundles`, `variant_slots`, `order_bump`, `upsell_offer`, `upsell_bundle_tiers`, `receipt_summary`, payment flags, and SDK 0.4.26 line-item personalization (`slot_personalization` for the MV per-slot picker, `order_personalization` for the order-wide default).
