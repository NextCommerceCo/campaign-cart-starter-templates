...
| `demeter` | Single-offer upsell page | `upsell_offer.type: single` on the shared `upsell/offer.html` shell from `src/demeter/upsell-single.html`; same contract as the apollo row (partial defaults `pricing_mode` full_price per the demeter brand contract; demo page opts into discounted). | Yes: same inputs as apollo. | Yes under promoted SDK lint. | Copy of the apollo partial; `countdown-timer.html` added alongside. | Done |
