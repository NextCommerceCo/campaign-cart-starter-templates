...
| `apollo` | Single-offer upsell page | Include: `upsell-single-offer.html` from `src/apollo/upsell-single.html` (`data-next-catalog-component="upsell-single-offer"`). One package, fixed price, accept or skip, no tiers; optional `voucher` routes through a hidden linked bundle selector; timer via `countdown-timer.html` in SDK `duration` mode. | Yes: `upsell_offer`, `offer_features`, `offer_comparison`, `offer_steps`, `guarantee_note`, `countdown_timer`, `announcement_text`. | Yes under promoted SDK lint (`upsell-single` page). | Demo copy is a protection plan; every string is a frontmatter override. Apollo only until propagated. | Done |
