...
| `apollo` | Composable upsell page (all upsell surfaces) | Shell include: `upsell/offer.html` (`data-next-catalog-component="upsell-offer"`, `offer_type` tier-cards \| tier-pills \| stepper \| single \| mv) dispatching `upsell-offer-context` / `-selector` / `-controls` / `-heading` / `-bullets` / `-prices` / `-media`, `upsell-cta`, `upsell-decline`, `upsell-video`, `upsell-guarantees`; page-level `upsell-announcement`, `upsell-header-bar` (steps \| logo), `upsell-hero` (band \| card), `upsell-body` (reviews \| features \| comparison \| steps), `upsell-secondary-offer` (cta \| card, proxy actions). Used from `src/apollo/upsell-bundle-stepper.html`, `upsell-bundle-tier-pills.html`, `upsell-bundle-tier-cards.html`, `upsell-single.html`, `upsell-vsl.html`. The v1 `upsell-bundle-*-offer.html` / `upsell-single-offer.html` files remain as deprecated shims. | Yes: `upsell_offer`, `upsell_bundle_tiers`, `upsell_layout`, `upsell_hero`, `offer_bullets`, `upsell_video`, `upsell_guarantees`, `upsell_body`, `upsell_secondary`, `announcement_text` / `announcement_variant`, `header_bar`, `countdown_timer`, `swiper_slides` / `swiper_thumbs` / `media_images`. | Yes under promoted/all-page SDK lint (all five apollo upsell pages). | Propagate the partial set to the other seven families (they still ship the v1 monolithic offer partials); MV `offer_type=mv` branch is written but only exercised once propagated to an MV family. | Next |
| `apollo` | Single-offer upsell page | `offer_type='single'` on `upsell/offer.html` from `src/apollo/upsell-single.html` + `upsell/body.html` sections + `upsell/secondary-offer.html` variant=card (proxy accept/skip → one SDK offer). Optional `voucher` routes through a hidden linked bundle selector outside the offer; timer via `countdown-timer.html` in SDK `duration` mode. `upsell-single-offer.html` is now a shim that maps the v1 keys (`offer_features`, `offer_comparison`, `offer_steps`, `guarantee_note`) onto the sections. | Yes: `upsell_offer`, `upsell_layout`, `upsell_hero`, `upsell_body`, `upsell_secondary`, `countdown_timer`, `announcement_text`. | Yes under promoted SDK lint (`upsell-single` page). | Demo copy is a protection plan; every string is a frontmatter override. `pricing_mode` discounted \| compare_at \| full_price \| unit_total \| unit_only. | Done |
| `apollo-mv-single-step` | Single-offer upsell page | Include: `upsell-single-offer.html` from `src/apollo-mv-single-step/upsell-single.html`; same contract as the apollo row (demo package 77 on the MV campaign, no voucher). | Yes: same inputs as apollo. | Yes under promoted SDK lint. | Copy of the apollo partial; `countdown-timer.html` added alongside. | Done |
| `olympus` | Single-offer upsell page | Include: `upsell-single-offer.html` from `src/olympus/upsell-single.html`; same contract as the apollo row. | Yes: same inputs as apollo. | Yes under promoted SDK lint. | Copy of the apollo partial; `countdown-timer.html` added alongside. | Done |
| `olympus-mv-single-step` | Single-offer upsell page | Include: `upsell-single-offer.html` from `src/olympus-mv-single-step/upsell-single.html`; same contract as the apollo row (demo package 77 on the MV campaign, no voucher). | Yes: same inputs as apollo. | Yes under promoted SDK lint. | Copy of the apollo partial; `countdown-timer.html` added alongside. | Done |
| `olympus-mv-two-step` | Single-offer upsell page | Include: `upsell-single-offer.html` from `src/olympus-mv-two-step/upsell-single.html`; same contract as the apollo row (demo package 77 on the MV campaign, no voucher). | Yes: same inputs as apollo. | Yes under promoted SDK lint. | Copy of the apollo partial; `countdown-timer.html` added alongside. | Done |
| `demeter` | Single-offer upsell page | Include: `upsell-single-offer.html` from `src/demeter/upsell-single.html`; same contract as the apollo row (partial defaults `pricing_mode` full_price per the demeter brand contract; demo page opts into discounted). | Yes: same inputs as apollo. | Yes under promoted SDK lint. | Copy of the apollo partial; `countdown-timer.html` added alongside. | Done |
| `shop-single-step` | Single-offer upsell page | Include: `upsell-single-offer.html` from `src/shop-single-step/upsell-single.html`; same contract as the apollo row. | Yes: same inputs as apollo. | Yes under promoted SDK lint. | Copy of the apollo partial; `countdown-timer.html` added alongside. | Done |
| `shop-three-step` | Single-offer upsell page | Include: `upsell-single-offer.html` from `src/shop-three-step/upsell-single.html`; same contract as the apollo row. | Yes: same inputs as apollo. | Yes under promoted SDK lint. | Copy of the apollo partial; `countdown-timer.html` added alongside. | Done |
