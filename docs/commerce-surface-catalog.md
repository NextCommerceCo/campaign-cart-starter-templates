...
| `apollo` | Checkout reveal and Apollo layout blocks | `checkout-apollo.js` + partials (`checkout-header`, `product-heading`, `promo-banner`, `promo-timer`, `as-seen-in`, `reviews-slider`, `guarantee`). | Yes: `checkout_reveal`, `checkout_reveal_cta`, plus layout object frontmatter. CSS promoted to `next-core.css`. | Yes under promoted/all-page SDK lint. | Keep checkout page `styles:` to CDN + route-specific only. | Done |
