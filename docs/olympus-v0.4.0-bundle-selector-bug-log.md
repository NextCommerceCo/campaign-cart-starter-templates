...
- Workaround (templates):
  - **No attribute-only** replacement through the same enhancer. Use **`data-summary-voucher-discounts`** + **`{discount.name}`** (and amount) in the summary template — already present above the badge in this checkout. Or small **custom JS** listening to cart/checkout updates to set badge text. Or **patch SDK** as above.
- Cross-ref:
  - Migration **§ Known #10**.
