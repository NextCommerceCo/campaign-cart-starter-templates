...
- **Receipt / checkout mobile summary (cart strip):** **`cart.hasDiscounts`** (plural — `CartDisplayEnhancer`; not **`cart.hasDiscount`**, which is **`package.*` / bundle naming**) + **`data-next-display="cart.subtotal"`** on the crossed line next to **`cart.total`**. Do **not** use **`data-next-display="cart.originalPrice"`** on **`cart.*`** — it is **not implemented** in current `CartDisplayEnhancer` (use **`cart.subtotal`** instead). Legacy **`cart.hasSavings`** / **`cart.compareTotal`** remain older naming only; prefer **`hasDiscounts`** / **`subtotal`** for new markup.
...
| `shop-single-step` | — (no bundle selector) | ✅ **0.4.x** | ✅ QA | **`sdk_version` `0.4.18`**. Shop flow — no bundle selector; cart pre-populated before checkout. Single long form. Cart summary: `cart-summary04.html` (shop-specific: image items + qty badge; coupon UI may not initialise — `CouponInputEnhancer` + `data-next-cart-summary` caveat). Upsell: standard bundle chain (stepper → tier-pills → tier-cards, matching olympus). **`checkout-header.html`** (`checkout-header--lg`) + **`checkout--shop`** on **`checkout.html`** **`page-wrapper`**. **`.checkout--shop`** / **`.checkout--shop-column-logo`** rules exist in every kit **`next-core.css`** for parity; only **`shop-*`** checkout HTML sets them on **`page-wrapper`**. |
| `shop-three-step` | — (no bundle selector) | ✅ **0.4.x** | ✅ QA | **`sdk_version` `0.4.18`**. Shop multi-step flow: `information.html` (email + shipping fields) → `shipping.html` (review + method selection) → `billing.html` (review + payment) → upsell chain → receipt. Step guard: `checkout-shop-three-billing.js` reads `sessionStorage` to block out-of-order page access. Cart summary: `cart-summary04.html` on all 3 checkout pages (same coupon / cart-summary caveat as `shop-single-step`). Shipping methods: `checkout-shop-three-shipping.js` calls `sdk.getShippingMethods()` and renders into `#next-shipping-options-container`. Billing hidden form (`.next-hidden-form`): preserves shipping fields for SDK submission. `window.CHECKOUT_CONFIG` injected inline on `shipping.html` (step 2) and `billing.html` (step 3). Upsell: standard bundle chain (stepper → tier-pills → tier-cards, matching olympus/shop-single-step). **Default demo:** logo in the left column only; **`checkout-header.html`** include is **commented out** in Liquid on each checkout page — **`page-wrapper`** uses **`checkout--shop checkout--shop-column-logo`** (latter restores **`padding: 1.25rem`** on **`.checkout__form-container--main`** under 991px / 479px when there is no top bar). To match **`shop-single-step`** top bar: uncomment the include, drop **`checkout--shop-column-logo`**, and add **`hide`** on the duplicate **`.checkout-header__brand`**. |
