...
**Reference:** `apollo/upsell-single.html` in [campaign-cart-starter-templates](https://github.com/NextCommerceCo/campaign-cart-starter-templates) — composed from the shared upsell partial set: `upsell/offer.html` with `upsell_offer.type: single` (the same shell the bundle upsells use with `stepper` / `tier-pills` / `tier-cards`), `upsell/body.html` sections (`features`, `comparison`, `steps`, `reviews`) and `upsell/secondary-offer.html` (closing card whose accept/skip proxy to the one SDK offer). Driven entirely by frontmatter (`upsell_offer`, `upsell_layout`, `upsell_hero`, `upsell_body`, `upsell_secondary`); the v1 keys `offer_features` / `offer_comparison` / `offer_steps` / `guarantee_note` still work through the `upsell-single-offer.html` shim. `apollo/upsell-vsl.html` shows the long-form VSL mix (hero card + countdown, `upsell_video`, `upsell_guarantees`, reviews). Set `upsell_offer.voucher` to apply one coupon to the accepted line: the partial then renders a hidden pre-selected bundle selector and reads `bundle.<selector_id>.*` prices instead of `package.*`. Keep `data-next-show` and `data-next-display` on separate elements.
