...
**`sdk_version`** — must be a **pinned semver string** from the starter reference (e.g. `"0.4.27"`), never `"latest"`. A wrong or stale version causes subtle Campaign Cart runtime behaviour with no obvious build error.
