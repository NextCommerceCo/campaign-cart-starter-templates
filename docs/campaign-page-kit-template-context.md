...
- **Identity/PII is opt-in; the `accepts_marketing` gate is per adapter.** The `*_enabled` flags send raw email/phone to the vendor via the core's `onContact` hook. `accepts_marketing` is the newsletter opt-in — the SDK only records it on the prospect cart; creation never depends on it — so the gate is decided per adapter: **ad-platform advanced matching** (TikTok, Snapchat, Pinterest) fires on prospect-cart creation only and only when the box is ticked (marketing use); **attribution vendors** (Northbeam `identify`, Triple Whale `Contact`) fire regardless of the box, and also as soon as a valid checkout email/phone is entered, because they join a backend order to the browser session by email — gating that on the newsletter box caps the attribution match rate at the newsletter opt-in rate. Leave the flags unset unless the campaign has a compliance-reviewed reason.
- **Northbeam on shop-sync stores**: the Shopify Connector order is the order of record and the funnel purchase can never match it (NEXT order id vs Shopify checkout token) — block the client-side purchases (`northbeam_blocked_events: "dl_purchase, dl_upsell_purchase"`), enable `northbeam_identity_enabled`, and rely on email identity. Same shape as the Triple Whale note below.
...
- **MV upsell per-slot fields are wired via `slot_personalization`** — the same frontmatter contract name as the MV checkout selector. `upsell/offer-selector.html` (the MV upsell's slot template) renders the configured `data-next-property` field inside the upsell slot template, so each accepted upsell unit can carry distinct text. It ships enabled on **`apollo-mv-single-step/upsell-mv.html`** and **`olympus-mv-single-step/upsell-mv.html`** (disabled on `olympus-mv-two-step`). **To turn it on for another MV upsell page, just set `slot_personalization.enabled: true` in that page's frontmatter** — the shared `upsell/offer-selector.html` partial renders the field from the same contract, so there's nothing to copy within an MV family. `order_personalization` remains available for the one-value-for-all fallback (rendered as a full-width field above the slot stage), and is the discoverable path for non-slot upsells (which have no slot stage to render per-slot fields); the MV upsell demo keeps it disabled because the reference uses per-slot fields. **If you enable both at once, give them distinct `property_key`s.** The accept merge is `{ ...default, ...slot }`, so a per-slot value overrides the page default on a shared key — the demo keys `order_personalization` `gift_message` to stay clear of the per-slot `upsell_message`.
...
**Reference:** `apollo/upsell-single.html` in [campaign-cart-starter-templates](https://github.com/NextCommerceCo/campaign-cart-starter-templates) — composed from the shared upsell partial set: `upsell/offer.html` with `upsell_offer.type: single` (the same shell the bundle upsells use with `stepper` / `tier-pills` / `tier-cards`), `upsell/body.html` sections (`features`, `comparison`, `steps`, `reviews`) and `upsell/closing-cta.html` (closing card whose accept/skip proxy to the one SDK offer). Driven entirely by frontmatter (`upsell_offer`, `upsell_layout`, `upsell_hero`, `upsell_body`, `closing_cta`). `apollo/upsell-vsl.html` shows the long-form VSL mix (hero card + countdown, `upsell_video`, `upsell_guarantees`, reviews). Set `upsell_offer.voucher` to apply one coupon to the accepted line: the partial then renders a hidden pre-selected bundle selector and reads `bundle.<selector_id>.*` prices instead of `package.*`. Keep `data-next-show` and `data-next-display` on separate elements.
