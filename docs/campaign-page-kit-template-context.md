...
**Per-campaign storage scope (SDK 0.4.34+)** — the SDK scopes cart/funnel/voucher storage per campaign automatically, keyed on the API key plus the **first URL path segment**. The kit's `/<slug>/<page>/` URL shape works with no extra config. The one layout that breaks the derivation is a funnel that **mixes path depths** — e.g. a landing page at `/hu/` with its checkout at `/hu/checkout` — which resolves to different scopes and drops the cart mid-funnel. If you deploy a funnel shaped like that, declare the scope explicitly with `window.nextConfig.storageScope` (set before the SDK loads, e.g. in `config.js`) or `<meta name="next-storage-scope" content="...">`.
